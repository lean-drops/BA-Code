...
Zentrale PySide6-GUI mit integriertem Scan:
  • Scan: ruft organizer.chronik.chronik_finder.run.run_finder(pdf_dir=…) direkt auf
           – Live-Logs im Fenster, Ordnerauswahl, Session/CSV-Autodetektion
...
Voraussetzungen:
  ...
  optional: pip install pymupdf pytesseract pillow pdfplumber pdfminer.six pypdf opencv-python scikit-image
    ...
    def set_scene_for_graph(self, G: nx.Graph, pos: Dict[str, Tuple[float, float]],
        ...
        for n, d in G.nodes(data=True):
            ...
            if role == "chronik":
                ...
                item.setBrush(QtGui.QColor("#1fb6ff")); pen = QtGui.QPen(QtGui.QColor("#083344")); pen.setWidth(1); pen.setCosmetic(True); item.setPen(pen)
            else:
                r = 10 + 3.0 * math.sqrt(max(1, int(d.get("items", 1))))
                poly = QtGui.QPolygonF([QtCore.QPointF(0, -r), QtCore.QPointF(-0.866*r, 0.5*r), QtCore.QPointF(0.866*r, 0.5*r)])
                item = QtWidgets.QGraphicsPolygonItem(poly)
                item.setBrush(QtGui.QColor("#fbbf24")); pen = QtGui.QPen(QtGui.QColor("#3a2c0b")); pen.setWidth(1); pen.setCosmetic(True); item.setPen(pen)
            item.setPos(x, y); item.setFlag(QtWidgets.QGraphicsItem.ItemIsMovable, True)
            item.setToolTip(f"{'Chronik' if role=='chronik' else 'PDF'}: {label}")
            scene.addItem(item)
            if show_labels:
                t = scene.addSimpleText(label); t.setBrush(QtGui.QColor("#e6e6e6"))
                t.setFlag(QtWidgets.QGraphicsItem.ItemIgnoresTransformations, True); t.setPos(x + r + 6, y - 8)
                self._nodes_by_label[label.lower()] = t
            self._nodes_by_label[label.lower()] = item
        for u, v, ed in G.edges(data=True):
            w = float(ed.get("weight", 1.0))
            if w < float(min_w): continue
            x1, y1 = pos.get(u, (0, 0)); x2, y2 = pos.get(v, (0, 0))
            path = QtGui.QPainterPath(QtCore.QPointF(x1, y1))
            dx = x2 - x1; k = 0.15*abs(dx)
            path.cubicTo(QtCore.QPointF(x1+0.25*dx, y1-k), QtCore.QPointF(x2-0.25*dx, y2+k), QtCore.QPointF(x2, y2))
            ...
            p.setPen(pen); p.setZValue(-1); scene.addItem(p)
        self.setScene(scene)
    ...
    def highlight(self, term: str) -> int:
        ...
        for key, item in self._nodes_by_label.items():
            ...
            if isinstance(item, (QtWidgets.QGraphicsEllipseItem, QtWidgets.QGraphicsPolygonItem)):
                pen = item.pen(); pen.setWidth(3 if on else 1); pen.setColor(QtGui.QColor("#ef4444") if on else pen.color()); item.setPen(pen); hits += int(on)
...
class NetworkTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.view = GraphView(); lay.addWidget(self.view, 1)
    ...
    def _choose_csv(self):
        ...
        if p: self.load_csv(p)
    def load_csv(self, path: str):
        self.df = read_csv_auto(path); set_last_csv(path); self._rebuild()
    def set_csv_from_outside(self, path: str):
        if path and os.path.isfile(path): self.load_csv(path)
    def _rebuild(self):
    ...
    def _export_png(self):
        if not get_last_csv():
            QtWidgets.QMessageBox.information(self, "Hinweis", "CSV zuerst laden."); return
...
class SimilarityTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.chk_tfidf = QtWidgets.QCheckBox("TF-IDF"); self.spin_shared = QtWidgets.QSpinBox(); self.spin_shared.setRange(0, 10000); self.spin_shared.setValue(1)
        ...
        for b in [self.btn_run, self.btn_export_pairs, self.btn_export_matrix, self.btn_report]: btns.addWidget(b)
        ...
        tabs.addTab(w2, "Nachbarn je Chronik"); lay.addWidget(tabs, 1)
        self.btn_load.clicked.connect(self._choose_csv); self.btn_run.clicked.connect(self._run)
        self.btn_export_pairs.clicked.connect(self._export_pairs); self.btn_export_matrix.clicked.connect(self._export_matrix); self.btn_report.clicked.connect(self._report)
        self._apply_glass_style(self)
    def _apply_glass_style(self, w: QtWidgets.QWidget):
        QtWidgets.QApplication.setStyle("Fusion")
        pal = QtGui.QPalette()
        pal.setColor(QtGui.QPalette.Window, QtGui.QColor(18,18,18)); pal.setColor(QtGui.QPalette.Base, QtGui.QColor(28,28,28)); pal.setColor(QtGui.QPalette.Text, QtGui.QColor(230,230,230))
        w.setPalette(pal)
        w.setStyleSheet("""
    ...
    def _run(self):
        if self.df is None:
            QtWidgets.QMessageBox.information(self, "Hinweis", "CSV zuerst laden."); return
        chronik_col = self.cb_chronik.currentText(); werk_col = self.cb_werk.currentText()
        ...
        S = cosine_similarity(M); self._S = S; nz = (M>0)
        rows = []
        n = S.shape[0]
        for i in range(n):
            sims = S[i, i+1:]; order = np.argsort(-sims)
        ...
        self.model_pairs.set_dataframe(df_pairs); self.cb_pick.clear(); self.cb_pick.addItems(self._chroniken); self.spin_neighbors.valueChanged.connect(lambda _: self._refresh_neighbors()); self.cb_pick.currentTextChanged.connect(lambda _: self._refresh_neighbors()); self._refresh_neighbors()
    ...
    def _report(self):
        ...
        rows = "\n".join(f"<tr><td>{esc(a)}</td><td>{esc(b)}</td><td>{sim:.3f}</td><td>{shared}</td></tr>" for a,b,sim,shared in pairs[["chronik_a","chronik_b","similarity","shared_werke"]].values)
...
h1,h2{{color:#cfe2ff}} table{{border-collapse:collapse;width:100%}} th,td{{border:1px solid #2a2f3a;padding:8px}} th{{background:#1b2030}}</style></head>
<body><h1>Chronik-Ähnlichkeiten</h1><p style="opacity:.8">Quelle: {esc(get_last_csv())}</p>
...
# ---------- Scan-Worker (intern, QThread) ----------
class _DualStream(io.TextIOBase):
    """Leitet writes an zwei Ziele + Qt-Signal weiter."""
    def __init__(self, orig, callback): super().__init__(); self._orig = orig; self._cb = callback; self._buf = ""
    ...
    def flush(self):
        self._orig.flush()
        if self._buf:
            try: self._cb(self._buf);
            except Exception: pass
            self._buf = ""

class ScanWorker(QtCore.QObject):
    log = QtCore.Signal(str)
    done = QtCore.Signal(str, str)  # session_dir, mentions_csv
    error = QtCore.Signal(str)

    def __init__(self, pdf_dir: Optional[str]):
        super().__init__()
        self._pdf_dir = Path(pdf_dir).resolve() if pdf_dir else None

    @QtCore.Slot()
    def run(self) -> None:
        """Startet run_finder im Thread und leitet stdout/stderr ins Log."""
        try:
            try:
                from organizer.chronik.chronik_finder.run import run_finder
            except Exception as e:
                raise ImportError(f"Import von run_finder fehlgeschlagen: {e}") from e

            # Stdout/Stderr im Worker umlenken
            orig_out, orig_err = sys.stdout, sys.stderr
            sys.stdout = _DualStream(orig_out, lambda s: self.log.emit(s.rstrip("\n")))
            sys.stderr = _DualStream(orig_err, lambda s: self.log.emit(s.rstrip("\n")))
            try:
                self.log.emit("[INFO] Starte Finder (interner Aufruf) …")
                session_dir, df, agg = run_finder(pdf_dir=self._pdf_dir)
            finally:
                sys.stdout.flush(); sys.stderr.flush()
                sys.stdout, sys.stderr = orig_out, orig_err

            if not session_dir:
                raise RuntimeError("Finder lieferte keine Session. Siehe Log oben.")

            # Mentions-CSV finden
            m = Path(session_dir) / "chroniken_mentions.csv"
            if not m.exists():
                # Suche zur Sicherheit
                cand = list(Path(session_dir).glob("*mentions*.csv"))
                if cand: m = cand[0]
            if not m.exists():
                raise FileNotFoundError(f"Mentions-CSV nicht gefunden in {session_dir}")

            self.log.emit(f"[INFO] Session-Ordner: {session_dir}")
            self.log.emit(f"[INFO] CSV geschrieben: {m}")
            self.done.emit(str(session_dir), str(m))
        except Exception as e:
            self.error.emit(f"[ERROR] Scan fehlgeschlagen: {e}")
...
class ScanTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.le_folder = QtWidgets.QLineEdit(); self.le_folder.setPlaceholderText("PDF-Ordner wählen …")
        ...
        for w in [self.le_folder, self.btn_folder, self.btn_run, self.btn_stop]: top.addWidget(w)
        lay.addLayout(top)
    ...
    def _run(self):
        if self._thread and self._thread.isRunning():
            QtWidgets.QMessageBox.information(self, "Hinweis", "Scan läuft bereits."); return
        folder = self.le_folder.text().strip() or None
        if folder and not os.path.isdir(folder):
            QtWidgets.QMessageBox.information(self, "Hinweis", "Ordner existiert nicht."); return
        ...
        # Aufräumen
        worker.done.connect(lambda *_: self._cleanup_thread(worker))
        worker.error.connect(lambda *_: self._cleanup_thread(worker))
        self.btn_run.setEnabled(False); self.btn_stop.setEnabled(True)
        self._append("> Starte Scan …")
        self._thread.start()

    def _cleanup_thread(self, worker: ScanWorker):
    ...
    def _stop(self):
        # Der interne Finder unterstützt kein hartes Cancel. Wir beenden nur den Thread nach Abschluss des aktuellen Schritts.
        self._append("[WARN] Abbruch angefordert. Vorgang wird nach aktuellem Schritt enden.")
        if self._thread:
            self._thread.requestInterruption()

    def _on_done(self, session_dir: str, mentions_csv: str):
        set_last_session(session_dir); set_last_csv(mentions_csv)
        self.lbl_last.setText(f"Letzte CSV: {mentions_csv}")
        self._append("> Fertig.")
    ...
    def _open_session(self):
        ...
        if not path or not os.path.isdir(path):
            QtWidgets.QMessageBox.information(self, "Hinweis", "Keine Session bekannt."); return
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _load_last_into_tabs(self):
        ...
        if not last or not os.path.isfile(last):
            QtWidgets.QMessageBox.information(self, "Hinweis", "Keine zuletzt genutzte CSV gefunden."); return
        self.net_tab.set_csv_from_outside(last); self.sim_tab.load_csv(last)
